...
class EightPuzzleSearchProblem(search.SearchProblem):
    ...
    def getCostOfActions(self, actions):
        ...
        """
        return len(actions) #since the cost is only 1 for all actions

    
    def getHeuristic(self,state):#no of misplaced cells
        current = 0
        h=0
        for row in range( 3 ):
            for col in range( 3 ):
                if current != state.cells[row][col] and state.cells[row][col]!=0:
                    h+=1
                current += 1
        return h
